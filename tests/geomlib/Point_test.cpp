...
TEST_CASE( "Point<double> default constructor initializes to zero", "[Point<double>]" )
{
    ...
    REQUIRE( point.x() == Catch::Approx( 0.0 ).epsilon( 0.0001 ) );
    REQUIRE( point.y() == Catch::Approx( 0.0 ).epsilon( 0.0001 ) );
}
...
TEST_CASE( "Point<double> parameterized constructor sets values correctly", "[Point<double>]" )
{
    ...
    REQUIRE( point.x() == Catch::Approx( 3.5 ).epsilon( 0.0001 ) );
    REQUIRE( point.y() == Catch::Approx( -2.1 ).epsilon( 0.0001 ) );
}
